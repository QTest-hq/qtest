...
// createBranch creates a git branch with the test files and pushes to GitHub
func (w *IntegrationWorker) createBranch(ctx context.Context, workspacePath, branchName string, testFiles []string) error {
	...
	if output, err := cmdEmail.CombinedOutput(); err != nil {
		log.Warn().Err(err).Str("output", string(output)).Msg("failed to set git email")
	}

	cmdName := exec.CommandContext(ctx, "git", "config", "user.name", "QTest AI")
	cmdName.Dir = workspacePath
	if output, err := cmdName.CombinedOutput(); err != nil {
		log.Warn().Err(err).Str("output", string(output)).Msg("failed to set git name")
	}
	...
	log.Info().Str("branch", branchName).Msg("pushed branch to remote")

	// Create pull request using gh CLI
	prTitle := fmt.Sprintf("QTest: Add generated tests (%s)", branchName)
	prBody := fmt.Sprintf("## Generated Tests\n\nThis PR contains %d test file(s) generated by QTest.\n\n### Test Files\n", len(testFiles))
	for _, testFile := range testFiles {
		relPath, _ := filepath.Rel(workspacePath, testFile)
		prBody += fmt.Sprintf("- `%s`\n", relPath)
	}
	prBody += "\n🤖 Generated with QTest"

	cmd = exec.CommandContext(ctx, "gh", "pr", "create", "--title", prTitle, "--body", prBody, "--head", branchName)
	cmd.Dir = workspacePath
	if output, err := cmd.CombinedOutput(); err != nil {
		log.Warn().Err(err).Str("output", string(output)).Msg("failed to create PR - you may need to create it manually")
		return nil // Don't fail the job if PR creation fails
	}
	log.Info().Str("branch", branchName).Msg("created pull request")
